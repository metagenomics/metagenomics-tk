...
while IFS= read -r genomes_to_compare; do 
    ...
    average_nucleotide_identity.py -m TETRA !{params.steps.dereplication.bottomUpClustering.additionalParams.pyani} --force -i . -o out &> error.log
    ANI1=$(cat out/*.tab | cut -f 2 | tail -n 1) 
    ANI2=$(cat out/*.tab | cut -f 3 | head -n 2 | tail -n 1)
...
chmod +rx $DIRECTORY
cat *_dir/out/*.tab > ${DIRECTORY}/matrix.tab
