...
MEGAHIT_OUTPUT_ASSEMBLY=${MEGAHIT_OUTPUT_DIR}/final.contigs.fa

if [ -s "${MEGAHIT_OUTPUT_ASSEMBLY}" ]; then
  ...
  transform.sh ${MEGAHIT_OUTPUT_ASSEMBLY} ${ASSEMBLY_OUTPUT} ${HEADER_MAPPING_OUTPUT} !{sample} !{task.cpus}
