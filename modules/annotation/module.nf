...
process pMMseqs2 {
   ...
   then
         ...
         flock ${LOCK_FILE} concurrentDownload.sh --output=${DATABASE}/!{dbType} \
            ...
            --s3FileCommand="s5cmd !{S5CMD_PARAMS} cp !{DOWNLOAD_LINK} mmseqs.!{dbType}.tar.zst && zstd --rm -T!{task.cpus} -d mmseqs.!{dbType}.tar.zst && tar -xvf mmseqs.!{dbType}.tar -C ${DATABASE}/!{dbType}" \
	    ...
	    --s5cmdAdditionalParams="!{S5CMD_PARAMS}" \
            --localCommand="zstd -T!{task.cpus} -d !{DOWNLOAD_LINK} -o mmseqs.!{dbType}.tar && tar -xvf mmseqs.!{dbType}.tar -C ${DATABASE}/!{dbType}" \
            --expectedMD5SUM=!{MD5SUM}
         mkdir -p ${DATABASE}/!{dbType}
          
          # Path of the newly downloaded database. A mmseqs2 database consists out of multiple files so a directory is create.
          # Our naming convention dictates that a database parameter name has to be the real database name. 
          # So !{dbType} can be used to create a directory named after the database and then again to reference the database itself. 
